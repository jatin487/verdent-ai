...
const COURSES = [
  {
    id: 1, emoji: '🤝', title: 'ASL Alphabet Basics',
    category: 'Sign Language', level: 'Beginner',
    badgeCls: 'badge-violet', progress: 72,
    lessons: 12, duration: '2h 30m',
    desc: 'Learn all 26 American Sign Language alphabet letters with real AI-powered feedback.',
  },
  {
    id: 2, emoji: '🔢', title: 'Numbers in Sign Language',
    category: 'Sign Language', level: 'Beginner',
    badgeCls: 'badge-violet', progress: 45,
    lessons: 8, duration: '1h 45m',
    desc: 'Master signing numbers 1–100 for everyday communication.',
  },
  {
    id: 3, emoji: '📖', title: 'Braille Introduction',
    category: 'Visually Impaired', level: 'Beginner',
    badgeCls: 'badge-sky', progress: 20,
    lessons: 10, duration: '3h 00m',
    desc: 'Audio-first course teaching Braille reading through sound and tactile description.',
  },
  {
    id: 4, emoji: '🗣️', title: 'Speech Therapy Exercises',
    category: 'Communication', level: 'Intermediate',
    badgeCls: 'badge-green', progress: 89,
    lessons: 15, duration: '4h 15m',
    desc: 'AI-guided exercises to improve pronunciation, fluency, and confidence.',
  },
  {
    id: 5, emoji: '🎨', title: 'Creative Arts & Expression',
    category: 'Arts', level: 'All Levels',
    badgeCls: 'badge-amber', progress: 35,
    lessons: 20, duration: '5h 00m',
    desc: 'Fully accessible art course adapted for motor and sensory differences.',
  },
  {
    id: 6, emoji: '💻', title: 'Coding with Accessibility',
    category: 'Technology', level: 'Intermediate',
    badgeCls: 'badge-sky', progress: 58,
    ...
    <main id="main-content" className="dashboard">
      <div className="container">
        ...
        <div className="dashboard__header fade-in-up">
          <div>
            <A11yText as="h1" className="dashboard__title">Your Learning Journey</A11yText>
            <p className="dashboard__subtitle">
              {COURSES.length} courses · Overall progress: <strong className="gradient-text">{totalProgress}%</strong>
            </p>
          </div>
          <div className="dashboard__overall-progress glass-card">
            <div className="dashboard__ring">
              <svg viewBox="0 0 64 64" aria-label={`${totalProgress}% overall progress`}>
          ...
          {visible.length === 0 ? (
            ...
            >
              <div className="course-card__top">
                <span className="course-card__emoji animate-float">{c.emoji}</span>
                <span className={`badge ${c.badgeCls}`}>{c.category}</span>
              </div>

              <A11yText as="h2" id={`course-title-${c.id}`} className="course-card__title">
                {c.title}
              </A11yText>
              ...
              <div className="course-card__meta">
                ...
                <span className={`badge ${c.level === 'Beginner' ? 'badge-green' : c.level === 'Intermediate' ? 'badge-amber' : 'badge-sky'}`}>
                  {c.level}
                </span>
              </div>
              ...
              <div className="course-card__progress">
                ...
                <div className="progress-bar" role="progressbar" aria-valuenow={c.progress} aria-valuemin={0} aria-valuemax={100}>
                  <div className="progress-bar__fill" style={{ width: `${c.progress}%` }} />
                </div>
              </div>

              <button className="btn btn-primary course-card__btn" id={`continue-course-${c.id}`}>
                {c.progress === 0 ? '▶ Start' : c.progress === 100 ? '✓ Review' : '▶ Continue'}
              </button>
            </article>
          ))}
        </div>
